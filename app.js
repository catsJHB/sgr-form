...
function closeTransaction() {
  ...
  // Show confirm screen immediately — never blocked by network
  ...
  // Submit to sheet separately — UI never waits for this
  submitToSheet(toSend);
}

function submitToSheet(toSend) {
  showStatus('Saving to spreadsheet…', 'sending');
  var promises = toSend.map(function(item) {
    var form = new FormData();
    form.append('data', JSON.stringify(item));
    return fetch(APPS_SCRIPT_URL, { method:'POST', body:form });
  });
...
function resetForm() {
  ...
  document.getElementById('fields-kg').style.display      = 'none';
  document.getElementById('fields-units').style.display   = 'none';
  document.getElementById('pgmCard').style.display        = 'none';
  document.getElementById('perUnitDisplay').style.display = 'none';
  document.getElementById('perKgDisplay').style.display   = 'none';
  ...
  // Force all steps hidden then show step 1 only
  document.getElementById('step1').classList.add('visible');
